...
class LITS_preprocess:
    ...
    def write_train_val_name_list(self):   # 生成train_name_list,val_name_list,而后通过调用write_name_list method 创建txt文件
    ...
    def write_name_list(self, name_list, file_name):    # 创建txt文件
...
if __name__ == '__main__':
    # raw_dataset_path = '/research/Data/ADNI100_HDBET_WYJ_test/git/3DUNet-Pytorch-master/rawdataset/train'
    # fixed_dataset_path = '/research/Data/ADNI100_HDBET_WYJ_test/git/3DUNet-Pytorch-master/fixed_lits'
    
    raw_dataset_path = '/research/Data/ADNI100_HDBET_WYJ_test/git/3DUNet-Pytorch-master/ADNi/AdniRowData/train'
    fixed_dataset_path = '/research/Data/ADNI100_HDBET_WYJ_test/git/3DUNet-Pytorch-master/ADNi/fixed_lits'
    args = config.args 
    tool = LITS_preprocess(raw_dataset_path,fixed_dataset_path, args)
    tool.fix_data()                            # 对原始图像进行修剪并保存
    tool.write_train_val_name_list()      # 创建 Train and Val索引txt文件
...
train_data = '/research/Data/ADNI100_HDBET_WYJ_test/git/3DUNet-Pytorch-master/ADNi/fixed_lits/ct'
dataname_list = os.listdir(train_data)
dataname_list.sort()
ori_data = sitk.ReadImage(os.path.join(train_data , dataname_list[2]))  # 读取一个数据
data1 = sitk.GetArrayFromImage(ori_data)  # 获取数据的array,(z,)
    

print(dataname_list[2], data1.shape)  #打印数据name、shape、某一个位置的元素的值（z,y,x）
